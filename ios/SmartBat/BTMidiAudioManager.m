...
@property(nonatomic) UInt8 patch;
@property(nonatomic) UInt8 bank;
@property(nonatomic) float volume;

@end

@implementation VoiceDef

@synthesize soundPath, name, patch, bank, volume;

@end


@implementation BTMidiAudioManager
...
- (void) configureForGeneralMidi: (NSString *) soundFont withSequence: (BTMidiSequence *) sequence {
    // For each program specified in the midi file create a new voice
    BTMidiChannelEvent * channelEvent;
    
    // Loop over all the events in the sequence
    for(BTMidiEvent * event in [sequence getSequence]) {
        // If it's a channel event check which type
        if(event.eventType == Channel) {
            channelEvent = (BTMidiChannelEvent *) event;
 ...
 */
-(AudioUnit) getSamplerForNote: (BTMidiNote *) note {
...
- (BOOL) createAUGraph {
    ...
    AUNode samplerNode;
    VoiceDef *voiceDef = [[VoiceDef alloc]init];
...
//        [wrapper getValue:&voiceDef];
        ...
        NSLog(@"voiceDef soundpath: %@", voiceDef.soundPath);
