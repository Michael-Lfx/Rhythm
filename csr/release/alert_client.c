 ...
 */
#ifdef ADD_DELAY_IN_READING
/* One extra timer will be used for adding delay in reading phone Alert 
 * characteristic.
 */
                                 ...
                                 LS_CONNECTION_PARAM_UPDATE_IND_T *p_event_data)
{
     ...
     */
     if(p_event_data->conn_interval < PREFERRED_MIN_CON_INTERVAL ||
        ...
        (int16)p_event_data->conn_latency < PREFERRED_SLAVE_LATENCY)
    {
        /* Delete timer if running */
        TimerDelete(g_app_data.conn_param_update_tid);
    ...
    switch(g_app_data.state)
    {
        ...
        case app_connected_discovering:
        {
            if(event_data->status == sys_status_success)
            {
                ...
                // MainEnableNotifications(g_app_data.st_ucid,
                        ...
                        (GATT_DISC_ALL_CHAR_DESC_CFM_T *cfm)
{
    ...
    if(g_app_data.conf_handle_ptr == NULL)
    {
        ...
        if(g_app_data.connected_device_profiles & ans_supported)
        {
             ...
             */
            ConfigureAlertNotificationServer(cfm);
        }
        else if(g_app_data.connected_device_profiles & pas_supported)
        {
    ...
    if (old_state != new_state)
    {
        ...
        switch (old_state)
        {
            ...
            case app_connected_discovering:
            {
                 ...
                 * application is implementing client role and has discovered the 
                ...
                if(new_state == app_connected)
                {
                    ...
                    /* Set the num of connection update attempts to one */
                    g_app_data.num_conn_update_req = 1;

                    /* Delete any connection parameter update request timer if 
                     * already running.
                     */
                    TimerDelete(g_app_data.conn_param_update_tid);
                    g_app_data.conn_param_update_tid = TIMER_INVALID;

                    /* Request a connection paramter update. */
                    requestConnParamUpdate();
                }
            }
            break;
 ...
 *
 *  DESCRIPTION
 *      This user application function is called whenever a system event is 
...
bool AppProcessLmEvent(lm_event_code event_code, LM_EVENT_T *event_data)
{
    switch (event_code)
    {
        ...
        case LM_EV_CONNECTION_COMPLETE: /* FALLTHROUGH */ 
